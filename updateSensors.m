function   sensorsStates = updateSensors(robot,mapa)
  ...
  for i=1:robot.numSensores
   ...
   if(sensorPosition(2)==0)
    pixel = mapa(48,sensorPosition(1));
   elseif(sensorPosition(2)==49)
    pixel = mapa(1,sensorPosition(1));
   else
      pixel = mapa(49-sensorPosition(2),sensorPosition(1)); %1 no mapa é preto
   end
  ...
  %encoders
  %motor 1
  sensorsStates(robot.numSensores+1) = (robot.velP(1) + robot.entreEixos*...
    robot.velP(2))/robot.raioRoda;
  sensorsStates(robot.numSensores+2) = (robot.velP(1) - robot.entreEixos*...
    robot.velP(2))/robot.raioRoda;
  %5% de chance de ler errado
  choice = rand(1,1);
  if(choice<0.05)
